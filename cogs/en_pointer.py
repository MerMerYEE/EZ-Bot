...
class Pointer(commands.Cog):
        ...
        if (ctx.message.mentions.__len__() > 0):
            for user in ctx.message.mentions:
                if os.path.isfile("./lib/servers/" + str(ctx.guild.id) + "/" + str(user.id) + ".txt"):
                    ...
                    pp = str(f.read())
                    f.close()
                    embed = discord.Embed(title="**" + user.name + "**'s Point", description= "**" + pp + " pp**",color=0xffffff)
            ...
            if os.path.isfile("./lib/servers/" + str(ctx.guild.id) + "/" + str(ctx.author.id) + ".txt"):
                ...
                pp = str(f.read())
                f.close()
                embed = discord.Embed(title="**" + ctx.author.name + "**'s Point", description= "**" + pp + " pp**",color=0xffffff)
        ...
        if (ctx.message.mentions.__len__() > 0):
            for user in ctx.message.mentions:
                if os.path.isfile("./lib/servers/" + str(ctx.guild.id) + "/" + str(user.id) + ".txt"):
                    ...
                    f.write(str(pp))
                    f.close()
                    embed = discord.Embed(title="**" + user.name + "**'s Point", description= "**" + str(pp) + " pp**",color=0xffffff)
                    pfp = str(user.avatar_url)
                    embed.set_thumbnail(url=pfp)
                    embed.set_footer(text="Offered by NACL - Shio", icon_url="https://raw.githubusercontent.com/Shio7/EZ-Bot/master/images/Shio7.png")
                    await ctx.trigger_typing()
            ...
            await ctx.trigger_typing()
            embed = discord.Embed(title="**Error!**", description= "Please mention the user to give Point", color=0xffffff)
            embed.set_thumbnail(url="https://raw.githubusercontent.com/Shio7/EZ-Bot/master/images/shio_error.png")
            embed.set_footer(text="Offered by NACL - Shio", icon_url="https://raw.githubusercontent.com/Shio7/EZ-Bot/master/images/Shio7.png")
            await ctx.send(embed=embed)
    ...
    async def on_command_error(error, ctx):
        if isinstance(error, commands.MissingPermissions):
                embed = discord.Embed(title="**Error!**", description= "You do not have permission to execute this command!", color=0xffffff)
                embed.set_thumbnail(url="https://raw.githubusercontent.com/Shio7/EZ-Bot/master/images/shio_error.png")
                embed.set_footer(text="Offered by NACL - Shio", icon_url="https://raw.githubusercontent.com/Shio7/EZ-Bot/master/images/Shio7.png")
                await ctx.send(embed=embed)
