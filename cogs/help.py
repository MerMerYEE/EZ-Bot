...
class Help(commands.Cog):
    ...
    async def 도움(self, ctx):
        ...
        embed.add_field(name='**계산**', value='**식을 풀어냅니다**', inline = False)
        ...
        embed.add_field(name='**포인트추가**', value='**``관리자 등급 필요`` 멘션된 유저에게 포인트를 부여합니다**', inline = False)
        embed.add_field(name='**지원 서버**', value='**[클릭!](https://discord.gg/HerTmj5)**', inline = False)
        embed.add_field(name='**음악**', value='**``connect, play, pause, resume, skip, volume(vol), stop, now_playing`` 음악 관련된 기능들입니다.**', inline = False)
